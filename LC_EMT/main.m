...
Function  = @(t, x, P) MutualInhibitionSelfExcitationa(t, x, P);
...
IndexMatrix = load('MISE_IndexMatrix_Gauss');
...
LC_Parameters.MaxIter = 50; %Set the maximum number of iterations for LC. 
LC_Parameters.ObjectiveLimit = -1; %Set a tolerance for the objective function, defined above. 
LC_Parameters.ub = 2*ones(1,14); %Bound parameter can be included to to maintain physiologically realistic parameter values. 
LC_Parameters.lb= zeros(1,14); %lower bound for the parameters. 
%These bounds can be modified to achieve better convergence.


LC_Parameters.InitParams = ones(1,14); %The initial parameter set for the model. 
...
% NonlinearConstraintParams.L = 20;
